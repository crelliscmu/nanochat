...
# Data mixture
parser.add_argument("--mmlu-epochs", type=int, default=0, help="number of epochs of MMLU in training mixture (teaches Multiple Choice)")
parser.add_argument("--gsm8k-epochs", type=int, default=0, help="number of epochs of GSM8K in training mixture (teaches Math and Tool Use)")
parser.add_argument("--tulu-long-rows", type=int, default=300, help="number of rows of allenai/tulu-v2-sft-long-mixture to add to mixture (0 = disabled)")
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(entity="789_project", project="sft-example", name=args.run, config=user_config)
...
get_report(model_tag=args.model_tag).log(section="SFT", data=[
