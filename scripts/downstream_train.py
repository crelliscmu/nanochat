...
Loads a pretrained (or already-SFT'd) checkpoint and fine-tunes it on a single
downstream dataset defined under tasks/. Saves into chatsft_checkpoints under
the provided --model-tag so the result can be loaded by
scripts.downstream_task_eval with `--source sft`.

Run as:

python -m scripts.downstream_train --model-tag downstream-mydataset

Or with torchrun for multi-GPU:

torchrun --standalone --nproc_per_node=8 -m scripts.downstream_train -- \
    --model-tag downstream-mydataset --device-batch-size=16
"""
...
from scripts.downstream_task_eval import run_chat_eval

from tasks.common import TaskMixture
# ---- TEMPLATE: import the downstream task class here ------------------------
# Copy tasks/downstream_template.py to tasks/<your_task>.py, rename the class,
# then import it here. Example:
#
# from tasks.my_downstream import MyDownstream
#
# The default below uses the bare template, which will fail at load-time until
# you fill in HF_DATASET / _extract_messages / evaluate.
from tasks.downstream_template import DownstreamTask
# -----------------------------------------------------------------------------

# =============================================================================
# TEMPLATE: adding a new downstream training dataset
#   1. Copy tasks/downstream_template.py → tasks/<your_task>.py and fill TODOs.
#   2. Replace the import above with your new class.
#   3. (Optional) Add a CLI flag below (see TEMPLATE CLI marker) to control
#      epochs or row-count so you can tune the mixture without editing code.
#   4. Reference your task in `train_tasks` (see TEMPLATE train-mixture marker).
#   5. (Optional) Add the held-out split to `val_tasks` (see TEMPLATE val marker)
#      so it contributes to validation bpb.
# =============================================================================
...
# Data mixture
parser.add_argument("--downstream-epochs", type=int, default=1, help="number of epochs of the downstream task in training mixture")
parser.add_argument("--downstream-rows", type=int, default=0, help="cap on rows from the downstream task per epoch (0 = use full split)")
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(entity="789_project", project="nanochat-downstream", name=args.run, config=user_config)
...
# Downstream data mixture and DataLoader
downstream_size = args.downstream_rows if args.downstream_rows > 0 else None
# ---- TEMPLATE train-mixture: build the training mixture here ----
# Default: N epochs of the single downstream task. Add auxiliary tasks
# (e.g. MMLU, SmolTalk) here if you want to mix in general data to combat
# catastrophic forgetting. Pass a task multiple times to oversample it.
train_tasks = [
    DownstreamTask(split="train", size=downstream_size)
    for _ in range(args.downstream_epochs)
]
# -----------------------------------------------------------------
train_dataset = TaskMixture(train_tasks)
print0(f"Training mixture: {len(train_dataset):,} rows (DownstreamTask x{args.downstream_epochs}, rows_per_epoch={downstream_size or 'all'})")

# ---- TEMPLATE val-mixture: held-out splits used for val bpb -----
val_tasks = [
    DownstreamTask(split="test"),
]
# -----------------------------------------------------------------
...
while True:
    ...
    # once in a while: estimate downstream task accuracy (all ranks participate)
    # use the original uncompiled model because the inputs keep changing shape
    if args.chatcore_every > 0 and (last_step or (step > 0 and step % args.chatcore_every == 0)):
        model.eval()
        engine = Engine(orig_model, tokenizer)
        # ---- TEMPLATE eval-during-training: register the downstream task name(s) ----
        # The name(s) below must be registered in scripts.downstream_task_eval's
        # task registry. Pick categorical vs sample limit based on the task's
        # eval_type (categorical => chatcore_max_cat, generative => chatcore_max_sample).
        downstream_tasks = ['Downstream']
        categorical_tasks = set()  # add any names whose eval_type == 'categorical'
        # -----------------------------------------------------------------------------
        task_results = {}
        ...
        mean_acc = sum(task_results.values()) / len(task_results)
        print0(f"Step {step:05d} | Downstream mean accuracy: {mean_acc:.4f}")
        wandb_run.log({
            "step": step,
            "total_training_flops": flops_so_far,
            "downstream/mean_accuracy": mean_acc,
            **{f"downstream/{task_name}": acc for task_name, acc in task_results.items()},
        })
        model.train()
...
get_report(model_tag=args.model_tag).log(section="Downstream training", data=[
