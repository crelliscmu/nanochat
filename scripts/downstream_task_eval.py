"""
Evaluate a downstream-task fine-tuned model.

The same generative/categorical evaluation loops used by chat_eval, but the
task registry below is scoped to downstream tasks. Reads the model trained by
scripts/downstream_train.py from chatsft_checkpoints (load via `--source sft`).

Example runs:
python -m scripts.downstream_task_eval -i sft -g downstream-mydataset -a Downstream
torchrun --nproc_per_node=8 -m scripts.downstream_task_eval -- -i sft -g downstream-mydataset -a Downstream
"""
...
# Replace the template import with your concrete downstream task class
# (the same one referenced from scripts/downstream_train.py).
from tasks.downstream_template import DownstreamTask
# ----------------------------------------------------------

# =============================================================================
# TEMPLATE: adding a new downstream evaluation task
#   1. Make sure your tasks/<name>.py subclass defines `eval_type` as either
#      "generative" (sampled completion + evaluate()) or "categorical"
#      (multiple-choice via letter logits; rows must expose a `letters` key).
...
#      marker) so it counts toward the aggregate downstream metric.
# =============================================================================
...
def run_chat_eval(task_name, model, tokenizer, engine,
    ...
    task_module = {
        # ---- TEMPLATE registry: register new downstream eval tasks here ----
        'Downstream': partial(DownstreamTask, split="test"),
...
if __name__ == "__main__":
    ...
    wandb_run = wandb.init(
        entity="789_project",
        project="downstream-evals",
        name=f"{args.source}/{model_slug}",
        config=vars(args),
        tags=[args.source, "downstream_eval"],
    ) if use_wandb else DummyWandb()

    # Get the tasks to evaluate on
    # ---- TEMPLATE aggregate: list all downstream eval names included by default ----
    all_tasks = ['Downstream']
    # all_tasks.append('MyNewEval')
    # --------------------------------------------------------------------------------
    baseline_accuracies = {
        # ---- TEMPLATE aggregate: random-guess baseline per task ----
        'Downstream': 0.0,  # 0.0 for generative, 1/N for N-way multiple choice
        # 'MyNewEval': 0.0,
        # ------------------------------------------------------------
    }
    ...
    get_report(model_tag=args.model_tag).log(section="Downstream evaluation " + args.source, data=[
        vars(args), # CLI args
        results,
        chatcore_metric_dict,
    ])
