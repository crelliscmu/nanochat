...
torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.base_train -- --depth=$DEPTH --model-tag $MODEL_TAG --target-param-data-ratio=9 --device-batch-size=16 --max-seq-len=$MAX_SEQ_LEN --fp8 --save-every-pct=25 --run=$WANDB_RUN
