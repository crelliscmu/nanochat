...
torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_sft -- --model-tag $MODEL_TAG --device-batch-size=8 --max-seq-len=$MAX_SEQ_LEN --tulu-long-rows=100000 --run=$WANDB_RUN
