...
class NanoChatRMSNorm(torch.nn.Module):
    """
    RMSNorm without learnable parameters, matching `nanochat.gpt.norm` which is just
    `F.rms_norm(x, (x.size(-1),))`. Kept as a module (no params) so existing HF
    infrastructure — tied weights, gradient checkpointing, `_init_weights` — keeps
    working unchanged.
    """
...
def rotate_half(x):
    ...
    NanoChat's `apply_rotary_emb` in `gpt.py` implements the rotation
    `(y1, y2) = (x1*c + x2*s, -x1*s + x2*c)`. Using HF's
    `q*cos + rotate_half(q)*sin` formulation, this matches when
    `rotate_half([x1, x2]) = [x2, -x1]` (note the sign flip relative to Llama).
    """
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
...
class NanoChatAttention(nn.Module):
    """Multi-headed attention matching `nanochat.gpt.CausalSelfAttention`.

    Projection order: Linear -> split heads -> (optional) RoPE -> QK RMSNorm ->
    scaled dot-product attention -> output projection. Supports GQA via
    `num_key_value_heads` and skips RoPE when `position_embeddings is None`
    (used for the DRoPE ablation).
    """
...
class NanoChatMLP(nn.Module):
    """SwiGLU MLP matching `nanochat.gpt.MLP`:

        down_proj( act(gate_proj(x)) * up_proj(x) )

    With `hidden_act="silu"` this reproduces the SwiGLU block from `gpt.py`.
    """
...
class NanoChatModel(NanoChatPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # DRoPE: when `use_rope=False` we skip rotary entirely. We pass
        # `position_embeddings=None` down the stack and NanoChatAttention handles it.
