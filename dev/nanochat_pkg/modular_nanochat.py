...
class NanoChatRMSNorm(Llama4TextL2Norm):
    """RMSNorm without learnable parameters (matches `nanochat.gpt.norm`)."""
...
def rotate_half(x):
    ...
    `nanochat/gpt.py` uses the rotation `(x1*c + x2*s, -x1*s + x2*c)`; expressed via the
    standard `q*cos + rotate_half(q)*sin` form this requires `rotate_half([x1, x2]) = [x2, -x1]`.
    """
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
...
class NanoChatMLP(LlamaMLP):
    """SwiGLU MLP: `down_proj( act(gate_proj(x)) * up_proj(x) )` matching `nanochat.gpt.MLP`."""
...
class NanoChatModel(LlamaModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # DRoPE: `config.use_rope=False` disables rotary entirely at runtime.
